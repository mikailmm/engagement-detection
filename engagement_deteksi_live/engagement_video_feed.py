...
with mp_face_mesh.FaceMesh(
    max_num_faces=1,
    refine_landmarks=False,  # default True # Mendapatkan 478 titik, termasuk iris
    ...
    # Variabel untuk logika fps
    prev_frame_time = 0
    new_frame_time = 0

    # Variabel untuk logika inferensi
    ...
    while cap.isOpened():
        ...
        # -------------------
        # Logika inferensi afektif emosi
        # -------------------

        landmarks_coords = []

        try:
            # Unpack object landmark ke array python
            ...
            if len(arr_hasil_deteksi) < jumlah_frame_mov_avg:
                ...
                cv2.putText(frame, f"Modus 5dtk: {mode_value}", (10, 90), font,
                            1, (100, 255, 0), 2, cv2.LINE_AA)
            ...
            te
            print('Wajah tidak terdeteksi', fps_text, end='\r')
            cv2.putText(frame, "Wajah tak terdeteksi", (10, 60), font,
                        1, (0, 0, 255), 2, cv2.LINE_AA)
            pass

        # --- 6. MENAMPILKAN HASIL FRAME ---
        cv2.imshow('Demo Deteksi Engagement', frame)

        # commented out karena halves the FPS lol
        # # Tampil mesh ketika 's' ditekan
